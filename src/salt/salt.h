...
protected:
    ...
    virtual bool Relax(shared_ptr<TreeNode> u, shared_ptr<TreeNode> v) = 0;  // from u to v
    virtual void DFS(shared_ptr<TreeNode> mstNode, shared_ptr<TreeNode> slNode, double eps) = 0;
};

class SaltBuilder : public SaltBase {
public:
    void Run(const Net& net, Tree& tree, double eps, bool refine = true);
